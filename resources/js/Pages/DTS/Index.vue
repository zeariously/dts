...
const assignmentIdOf = (doc) => {
    const value = Number(doc?.assignment_id || 0)
...
const documentNumberDisplay = (doc) => {
    ...
    const suffix = String(doc?.assignment_suffix || '')
        .trim()
        .toUpperCase()

    if (assignmentIdOf(doc) && suffix) {
        return `${doc.IDdoc}-${suffix}`
    }
...
const documentUrl = (doc) => {
    ...
    const assignmentId = assignmentIdOf(doc)

    return assignmentId
        ? `/dts/${doc.IDdoc}?assignment_id=${assignmentId}`
        : `/dts/${doc.IDdoc}`
}

const workflowActionUrl = (doc, action) => {
    if (!doc?.IDdoc) {
        return '/dts'
    }

    return `/dts/${doc.IDdoc}/${action}`
}

const workflowActionPayload = (doc, extra = {}) => {
    return {
        ...extra,
        assignment_id: assignmentIdOf(doc),
    }
}
...
const receiveTransferredDocument = (doc) => {
    ...
    router.post(
        workflowActionUrl(doc, 'receive'),
        workflowActionPayload(doc),
        {
            preserveScroll: true,
        onSuccess: () => {
            router.reload({
...
const confirmPendingAction = () => {
    ...
    const selectedDocument = selectedPendingDocument.value
    const endpoint = pendingActionType.value === 'receive'
        ? workflowActionUrl(selectedDocument, 'receive')
        : workflowActionUrl(selectedDocument, 'pullout')

    router.post(endpoint, workflowActionPayload(selectedDocument), {
        preserveScroll: true,
        onSuccess: () => {
            showPendingActionModal.value = false
            selectedPendingDocument.value = null
